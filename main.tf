...
resource "azurerm_virtual_machine_extension" "testextension" {
      ...
      settings = <<SETTINGS
      {
	"commandToExecute": "yum install -y wget && yum install -y git && yum -y update && yum install -y java-1.8.0-openjdk && sudo wget /etc/yum.repos.d/jenkins.repo http://pkg.jenkins-ci.org/redhat/jenkins.repo && sudo rpm https://jenkins-ci.org/redhat/jenkins-ci.org.key && sudo yum install -y jenkins"
      }
    SETTINGS
