...
resource "azurerm_virtual_machine_extension" "testextension" {
      ...
      settings = <<SETTINGS
      {
	"commandToExecute": "yum install -y wget && yum install -y git && yum -y update && yum install -y java-1.8.0-openjdk && 'sudo wget --user=root --password='Password1234!' -O /etc/yum.repos.d/jenkins.repo https://pkg.jenkins.io/redhat-stable/jenkins.repo' && 'rpm --import https://pkg.jenkins.io/redhat-stable/jenkins.io.key' && yum install -y jenkins && systemctl status jenkins && systemctl start jenkins && systemctl enable jenkins"
      }
    SETTINGS
